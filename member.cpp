...
void Member::like_book(Book *B1, time_t _time, bool first_time)
{
	...
	for (int i = 0; i < books.size(); i++)
		...
		{
			if (first_time == true)
				cout << "The book : " << B1->get_name() << " has been added to favorites" << endl;
...
void Member::follow(Member *m1, bool first_time)
{
	...
	for (int i = 0; i < friends.size(); i++)
		...
		{
			if (first_time == true)
				cout << m1->get_name() << " is already your friend" << endl; // do nothing
			return;
		}
	if (first_time == true)
		cout << m1->get_name() << " is added to your friends" << endl;
...
void Member::show_shelf(string shelf_name)
{
	...
	if (shelf_found == false)
		cout << "There is no shelf named : " << shelf_name << endl
			 << endl;
	else
	{
